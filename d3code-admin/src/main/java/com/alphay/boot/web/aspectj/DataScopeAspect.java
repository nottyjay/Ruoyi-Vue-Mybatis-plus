package com.alphay.boot.web.aspectj;
...
import com.alphay.boot.web.security.context.PermissionContextHolder;
    ...
    for (SysRole role : user.getRoles()) {
      ...
      if (DATA_SCOPE_ALL.equals(dataScope)) {
        ...
        if (StringUtils.isNotEmpty(userAlias)) {
          sqlString.append(
              StringUtils.format(
                  " OR {} IN (SELECT user_id FROM sys_user where dept_id = {})",
                  userAlias,
                  user.getDeptId()));
        }
        ...
        if (StringUtils.isNotEmpty(userAlias)) {
          sqlString.append(
              StringUtils.format(
                  " OR {} IN (SELECT user_id FROM sys_user WHERE dept_id IN ( SELECT dept_id FROM sys_dept WHERE dept_id = {} or find_in_set( {} , ancestors ) ))",
                  userAlias,
                  user.getDeptId(),
                  user.getDeptId()));
        }
