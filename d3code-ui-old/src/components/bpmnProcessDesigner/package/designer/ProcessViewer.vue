...
export default {
  ...
  dicts: ['bpm_process_instance_result'],
  props: {
    value: {  // BPMN XML 字符串
      type: String,
    },
      ...
      if (!this.elementOverlayIds[element.id] && element.type !== "bpmn:Process") {
        ...
        if (element.type === 'bpmn:StartEvent' && this.processInstance) {
          ...
          html = `<p>审批人：${task.assigneeUser.nickname}</p>
                  ...
                  <p>结果：${this.selectDictLabel(this.dict.type.bpm_process_instance_result, task.result)}</p>
          ...
          html = `<p>结果：${this.selectDictLabel(this.dict.type.bpm_process_instance_result, this.processInstance.result)}</p>`;
