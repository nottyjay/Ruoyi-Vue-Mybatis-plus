    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="流程分类" prop="category">
        <el-select v-model="queryParams.category" placeholder="流程分类" clearable style="width: 240px">
          <el-option v-for="dict in dict.type.bpm_model_category"
                     :key="dict.value"
                     :label="dict.label"
                     :value="dict.value"
          />
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="110px">
        ...
        <el-form-item v-if="form.id" label="流程分类" prop="category">
          <el-select v-model="form.category" placeholder="请选择流程分类" clearable style="width: 100%">
            <el-option v-for="dict in dict.type.bpm_model_category" :key="dict.value" :label="dict.label"
                       :value="dict.value"
            />
          </el-select>
        </el-form-item>
        ...
        <div v-if="form.id">
          <el-form-item label="表单类型" prop="formType">
            <el-radio-group v-model="form.formType">
              <el-radio v-for="dict in dict.type.bpm_model_form_type" :key="parseInt(dict.value)"
...
import { getBpmForm, getSimpleForms } from '@/api/bpm/bpmForm'
...
export default {
  name: 'model',
  dicts: ['bpm_model_form_type', 'bpm_model_category'],
  components: {
    Parser,
    Treeselect,
  ...
  data() {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/bpm/model/import',
        // 表单
        form: {},
        // 校验规则
        rules: {
  ...
  methods: {
    ...
    handleDesign(row) {
      this.$router.push({
        path: '/flowable/bpm/model/design',
        query: {
          modelId: row.id
        }
      })
    ...
    handleFormDetail(row) {
      // 流程表单
      if (row.formId) {
        getBpmForm(row.formId).then(response => {
          // 设置值
