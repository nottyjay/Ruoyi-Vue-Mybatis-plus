<template>
  <div class="app-container">
    ...
    <el-card class="box-card" v-loading="processInstanceLoading">
      ...
      >
        <div>
          <parser :config="detailForm" :key="new Date().getTime()" :disabled="true"
                  :need-submit="false"
          />
        </div>
      </el-col>
    ...
    getDetail() {
      ...
      getProcessInstance(this.id).then(response => {
        ...
        if (this.processInstance.processDefinition.formType === 10) {
          this.detailForm = {
            disabled: true, // 表单禁用
            needSubmit: false, // 按钮隐藏
            formTemplate: {
              config: JSON.parse(this.processInstance.processDefinition.formConf),
              list: decodeFields(this.processInstance.processDefinition.formFields)
            },
            models: this.processInstance.formVariables // 设置表单的值
          }
        }
