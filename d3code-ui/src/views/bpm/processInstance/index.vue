    ...
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="请选择状态" clearable>
          <el-option v-for="dict in dict.type.bpm_process_instance_status"
                     :key="dict.value" :label="dict.label" :value="dict.value"/>
        </el-select>
      </el-form-item>
      <el-form-item label="结果" prop="result">
        <el-select v-model="queryParams.result" placeholder="请选择流结果" clearable>
          <el-option v-for="dict in dict.type.bpm_process_instance_result"
                     :key="dict.value" :label="dict.label" :value="dict.value"/>
        </el-select>
      </el-form-item>
      <el-form-item>
...
<script>
import { listProcessInstance, cancelProcessInstance } from "@/api/bpm/processInstance";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      listProcessInstance(this.queryParams).then(response => {
    ...
    /** 新增按钮操作 **/
    handleAdd() {
      this.$router.push({ path: "/flowable/task/create"})
    },
    /** 取消按钮操作 */
    handleCancel(row) {
      const id = row.id;
    ...
    /** 处理详情按钮 */
    handleDetail(row) {
      this.$router.push({ path: "/flowable/task/detail", query: { id: row.id}});
    },
  }
};
</script>
